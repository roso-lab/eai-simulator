...
**v0.1.0** is the current EAI Simulator documentation release.

:doc:`Open the v0.1.0 documentation <index_en>`
...
For example, the ``EAI_hmrs`` Python package version comes from ``__version__`` in
``source/EAI_hmrs/EAI_hmrs/__init__.py``, and ``source/EAI_hmrs/setup.py`` reuses that same value.
