...
| `mushr_v2` | MuSHR Nano v2 Ackermann base | `MUSHR_ACKERMANN_CFG` | 内置相机、Orsus、LiDAR、keyboard、ROS | JSON / Env DIY |
...
1. **Env DIY 创建自定义环境** — 不传 `--env` 启动 `simulator.py`，先进入自定义 env 流程：
  ...
  ```
  启动后会提示选择 env 制定方式：
  - `1. 可视化窗口`：通过 Env DIY 窗口按 `Scenes → Robots → Payloads → Tools` 选择环境；Payloads 下分为 Manipulators（UR5/Z1）和 Sensors（Orsus/LiDAR），Tools 提供 Camera/Keyboard/ROS。Camera Tool 独立控制 Iris、Pegasus、CF2X、MuSHR 的内置单目相机和 Orsus 相机的 ROS 图像发布；ROS Tool 控制三种无人机的 LiDAR、IMU、GPS、磁力计和气压计，以及 Orsus 的 LiDAR 点云、里程计和 scan 发布。配置可保存为 `source/EAI_hmrs/EAI_hmrs/envs/<env_name>.json`。
...
4. 切换到“Payloads”，在 “Manipulators” 子页选择 UR5/Z1，或在 “Sensors” 子页选择 Orsus/LiDAR；再切换到 “Tools” 选择 Camera/Keyboard/ROS。Camera Tool 独立控制 Iris、Pegasus、CF2X、MuSHR 的内置单目相机和 Orsus 相机的 ROS 图像发布；ROS Tool 控制三种无人机的 LiDAR、IMU、GPS、磁力计和气压计，以及 Orsus 的 LiDAR 点云、里程计和 scan 发布。选中机器人后，不兼容、已添加或已有另一种机械臂的卡片会显示为不可拖动状态。
