...
[[noreturn]] void exit_without_global_dtors(int code) {
    // This binary can load system FCL/OctoMap together with cmeel OctoMap
    // through Crocoddyl/Pinocchio. Their global destructors double-free on exit.
    std::cout << std::flush;
    std::cerr << std::flush;
    std::_Exit(code);
}
...
int main(int argc, char* argv[]) {
    ...
    try {
      ...
      if (vm.count("help") != 0u) {
        std::cout << desc << "\n";
        exit_without_global_dtors(0);
      }
      po::notify(vm);
    } catch (po::error& e) {
      std::cerr << e.what() << std::endl << std::endl;
      std::cerr << desc << std::endl;
      exit_without_global_dtors(1);
    }
    ...
    for (const auto &robot_node : env["robots"]) {
        ...
        const float collisionRadius = robot_node["radius"]
            ? robot_node["radius"].as<float>()
            : -1.0f;
        std::shared_ptr<Robot> robot = create_robot(
            robotType, position_bounds, collisionRadius);
        ...
        if (iter == robot_motions.end()) {
            ...
            std::ifstream is(motionsFile, std::ios::in | std::ios::binary);
            if (!is) {
                throw std::runtime_error("Cannot open motion primitives: " + motionsFile);
            }
            const std::vector<std::uint8_t> bytes(
                (std::istreambuf_iterator<char>(is)),
                std::istreambuf_iterator<char>());
            const auto motion_data = nlohmann::json::from_msgpack(bytes);
            load_motions(motion_data, robot, robotType, env_min.size(), /*delta, filterDuplicates, alpha,*/ robot_motions[robotType]);
