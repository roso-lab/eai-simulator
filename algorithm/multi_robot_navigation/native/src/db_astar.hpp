...
void load_motions(
  const nlohmann::json& motion_data,
  ...
  Motions& result)
{
    ...
    if (!motion_data.is_array()) {
      throw std::runtime_error("Motion primitive data must be an array");
    }

    for (const auto& item : motion_data) {
      Motion m;
      m.aabb = fcl::AABB(fcl::Vector3f(0,0,0));
      if (!item.is_object()) {
        throw std::runtime_error("Motion primitive entry must be an object");
      }
      for (const auto& state_data : item.at("states")) {
        ob::State* state = si->allocState();
        const auto reals = state_data.get<std::vector<double>>();
        si->getStateSpace()->copyFromReals(state, reals);
      ...
      for (const auto& action_data : item.at("actions")) {
        oc::Control *control = si->allocControl();
        const auto reals = action_data.get<std::vector<double>>();
