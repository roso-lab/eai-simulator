...
std::unique_ptr<Model_robot> robot_factory(const char *file,
  ...
  if (dynamics == "unicycle1") {
    ...
    Integrator2_2d_params params(file);
    if (collision_radius > 0.0) {
      params.shape = "sphere";
      params.radius = collision_radius;
    }
    return std::make_unique<Integrator2_2d>(params, p_lb, p_ub);
...
joint_robot_factory(const std::vector<std::string> &robot_types,
                    ...
                    const Eigen::VectorXd &p_ub,
                    const std::vector<double> &collision_radii) {

  std::vector<std::string> robotParams;
  std::vector<std::shared_ptr<Model_robot>> jointRobot;
  for (size_t index = 0; index < robot_types.size(); ++index) {
    const auto &robot_type = robot_types[index];
    const double radius = index < collision_radii.size()
        ? collision_radii[index]
        : -1.0;
    jointRobot.push_back(
        robot_factory(
            (base_path + robot_type + ".yaml").c_str(), p_lb, p_ub, radius));
  }
